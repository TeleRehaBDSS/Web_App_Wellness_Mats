...
def process_fga_step_over_obstacle(csv_path: str, participant: str, manual_input: Optional[str] = None) -> Dict:
    ...
    if manual_score is not None:
        score = manual_score
        if score == 3:
            explanation_parts.append("Normal: Steps over obstacle without changing gait speed, no evidence of imbalance.")
        elif score == 2:
            explanation_parts.append("Mild impairment: Steps over obstacle without changing gait speed, no evidence of imbalance.")
        elif score == 1:
            explanation_parts.append("Moderate impairment: Steps over obstacle but must slow down and adjust steps to clear safely.")
        else:
            explanation_parts.append("Severe impairment: Cannot perform without assistance.")
    else:
        # Automatic scoring
        if speed_maintained and metrics["max_deviation_cm"] <= 15.24:
            score = 3
            explanation_parts.append(
                f"Normal: Steps over obstacle without changing gait speed, no evidence of imbalance."
            )
        elif speed_maintained:
            score = 2
            explanation_parts.append(
                f"Mild impairment: Steps over obstacle without changing gait speed, no evidence of imbalance."
            )
        else:
            score = 1
            explanation_parts.append(
                f"Moderate impairment: Steps over obstacle but must slow down and adjust steps to clear safely."
            )
